...
- [ ] **Dashboard check.** Open `http://{{VPS_HOST}}.tail<id>.ts.net:3848/` from any tailnet device. Expect a row for `<name>` showing green / heartbeat in the last 5 min / agent uptime / claude version. If missing: see [RUNBOOK.md](RUNBOOK.md) §"Dashboard not loading".
